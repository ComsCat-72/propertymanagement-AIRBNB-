...
function AnalyticsPage() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [{ data: props }, { data: views }] = await Promise.all([
      ...
      return { props: props ?? [], views: views ?? [] };
    },
  });
  ...
  if (!unlocked) {
    return (
      <div className="rounded-3xl border border-dashed border-border p-12 text-center">
        ...
        <p className="mt-4 text-lg font-bold">Analytics is a paid feature</p>
        <p className="mt-1 text-sm text-muted-foreground">Upgrade to Tier 1 or Tier 2 to see views per listing, trends and category breakdowns.</p>
        <Link to="/dashboard/billing">
          <Button className="mt-5 rounded-full bg-brand text-brand-foreground hover:bg-brand/90">See plans</Button>
        </Link>
      </div>
    );
  }
